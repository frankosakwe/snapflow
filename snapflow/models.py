...
class Snapshot(Base):
    ...
    snapshot_name = sa.Column(
        ...
        nullable=False,
        index=True,
        doc="User-friendly snapshot name"
    )
    project_name = sa.Column(
        sa.String(255),
        nullable=False,
        index=True,
        doc="Project this snapshot belongs to"
    )
    hash = sa.Column(
        sa.String(32),
        nullable=False,
    ...
    worker_pid = sa.Column(
        sa.Integer,
        nullable=True,
        doc="PID of background worker copying slave databases"
    )
    description = sa.Column(
        sa.Text,
        nullable=True,
        doc="Optional description of the snapshot"
    )
...
class DatabaseTable(Base):
    ...
    id = sa.Column(
        sa.Integer,
        sa.Sequence('table_id_seq'),
        primary_key=True,
        doc="Unique table identifier"
    )
    table_name = sa.Column(
        sa.String(255),
        nullable=False,
        doc="Original database name"
    )
    ...
    size_bytes = sa.Column(
        sa.BigInteger,
        nullable=True,
        doc="Estimated size of the database in bytes"
    )
...
sa.Index(
    'idx_snapshot_project_name',
    Snapshot.project_name,
    Snapshot.snapshot_name
)
sa.Index(
    'idx_snapshot_created',
