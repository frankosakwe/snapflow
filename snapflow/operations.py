...
def _copy_mysql_database(
    ...
    result = connection.execute(
        sa.text(f'SHOW TABLES IN `{source_database}`')
    )
    ...
    for table_name in tables:
        ...
        connection.execute(sa.text(
            f'ALTER TABLE `{target_database}`.`{table_name}` DISABLE KEYS'
        ))
        ...
        # Re-enable keys
        connection.execute(sa.text(
            f'ALTER TABLE `{target_database}`.`{table_name}` ENABLE KEYS'
        ))
...
def rename_database(
    ...
    if dialect == 'postgresql':
        ...
        for table_name in tables:
            connection.execute(sa.text(
                f'RENAME TABLE `{old_name}`.`{table_name}` '
                f'TO `{new_name}`.`{table_name}`'
            ))
