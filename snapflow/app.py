...
class DatabaseOperations:
    ...
    def __init__(self, connection, config):
        ...
        self.terminate_connections = partial(
            terminate_database_connections, connection
        )
...
class SnapFlow:
    ...
    def initialize_database(self) -> None:
        ...
        self.metadata_engine = create_engine(
            self.config['snapflow_url'],
            echo=False
        )
        ...
        for table in snapshot.tables:
            ...
            if not self.operations.database_exists(slave_name):
                raise SnapshotRestoreError(
                    ...
                    f"Slave database {slave_name} does not exist"
                )
    ...
    def cleanup_orphaned_databases(
        ...
        for db_name in all_dbs:
            if (db_name.startswith('snapflow_') and
                db_name != 'snapflow_data' and
                db_name not in tracked_dbs):
    ...
    def default_snapshot_name(self) -> str:
        ...
        """
        n = 1
        while self.session.query(Snapshot).filter(
